...
var commands = {
	...
	verify: function (message, words) {
		if (message.channel.permissionsOf(message.author).hasPermission('manageChannel')) {
			if(config.lvadmin) {
				const userName = "_discord_"+message.channel.server.id;
				if(config.lvtokens[message.channel.server.id]) {
					sendReply(message, `Please add the user ${integrationName} as a manager in your channel settings.`);
				} else {
					request.post("https://"+config.lvdomain+"/api/token", {form: {token: config.lvadmin, user: userName, duration: 3600*24*365*10}}, (err, res, body) => {
						if(err) {
							sendReply(message, "An error occurred: "+res.statusCode);
						} else {
							const response = JSON.parse(body);
							config.lvtokens[message.channel.server.id] = body.token;
							saveTokens();
							sendReply(message, `Please add the user ${integrationName} as a manager in your channel settings.`);
						}
					})
